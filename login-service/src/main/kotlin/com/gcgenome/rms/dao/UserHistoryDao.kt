...
interface UserHistoryDao {
    fun DSLContext.insertUserHistoryByPasswordReissue(userHistory: UserHistoryDTO): Mono<UserHistory> {
        return Mono.from(
            insertInto(USER_HISTORY)
                .set(USER_HISTORY.USER_ID, userHistory.userId)
                .set(USER_HISTORY.CHANGED_BY, "Password reissue")
                .set(USER_HISTORY.CHANGED_AT, LocalDateTime.now())
                .set(USER_HISTORY.FIELD_NAME, "password")
