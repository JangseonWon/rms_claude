...
interface ItemDao {
    ...
    fun DSLContext.selectItemBySampleIdAndStatus(orderId: UUID, sampleId: UUID, status: Status): Mono<Item> =
        Mono.from(
            select(
                ITEM.SERVICE_ID,
                ITEM.SERIAL,
                ...
                .where(SAMPLE.ORDER_ID.eq(orderId).and(SAMPLE.ID.eq(sampleId)).and(SAMPLE.STATUS.eq(status.toString())))
        ).map(Item::toModel)
}
