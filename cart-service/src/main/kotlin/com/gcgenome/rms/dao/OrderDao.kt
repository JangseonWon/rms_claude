...
interface OrderDao {
    fun DSLContext.selectOrderById(orderId: UUID, sampleId: UUID, serviceId: String, status: Status): Mono<Order> =
        Mono.from(
            select(
                ORDER.SERIAL,
                ORDER.CREATE_AT,
                ...
                .where(REQUEST.ORDER_ID.eq(orderId).and(REQUEST.SAMPLE_ID.eq(sampleId))
                    .and(REQUEST.SERVICE_ID.eq(serviceId)).and(REQUEST.STATUS.eq(status.toString())))
