...
interface UserServiceDao {
    fun DSLContext.selectUserServiceById(userId: String, serviceId: String): Mono<UserService> {
        return Mono.from(selectFrom(USER_SERVICE).where(USER_SERVICE.USER_ID.eq(userId).and(USER_SERVICE.SERVICE_ID.eq(serviceId)))).map { it.into(UserService::class.java) }
    }
