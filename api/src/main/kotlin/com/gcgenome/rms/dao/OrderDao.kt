...
interface OrderDao{
    ...
    fun DSLContext.insertOrder(userId: String, serial:String): Mono<Order> {
        return Mono.from(
            insertInto(ORDER)
                .set(ORDER.ID,UUID.randomUUID())
                .set(ORDER.SERIAL,serial)  // 수정
    ...
    fun DSLContext.selectOrderSerial(serialPrefix:String): Mono<String> {
        return Mono.from(
            select(ORDER.SERIAL)
                .from(ORDER)
                .where(ORDER.SERIAL.like("$serialPrefix%"))
    ...
    fun DSLContext.selectOrderById(orderId: UUID): Mono<Order>{
        return Mono.from(
            select(
                ...
                .leftOuterJoin(SAMPLE_TYPE).on(SAMPLE_TYPE.ID.eq(SAMPLE.SAMPLE_TYPE_ID))
