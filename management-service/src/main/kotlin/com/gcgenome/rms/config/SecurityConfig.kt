    ...
    fun resourceFilterChain(http: ServerHttpSecurity): SecurityWebFilterChain {
        ...
        return http {
            ...
            authorizeExchange {
                ...
                authorize (ServerWebExchangeMatchers.pathMatchers(HttpMethod.GET, "/actuator/health/**", "/swagger-ui/**", "/users/api-docs/**", "/v3/api-docs/**", "/users/webjars/**"), permitAll)
                authorize (ServerWebExchangeMatchers.pathMatchers(HttpMethod.PUT, "/w-api/**"), permitAll)
                authorize (ServerWebExchangeMatchers.pathMatchers(HttpMethod.POST, "/w-api/**"), permitAll)
                authorize (ServerWebExchangeMatchers.pathMatchers(HttpMethod.PATCH, "/w-api/**"), permitAll)
                authorize (ServerWebExchangeMatchers.pathMatchers(HttpMethod.DELETE, "/w-api/**"), permitAll)
