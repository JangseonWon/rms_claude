...
interface ServiceSampleTypeDao {
    fun DSLContext.selectServiceSampleTypeById(sampleTypeId: String, serviceId: String): Mono<ServiceSampleType>{
        return Mono.from(selectFrom(SERVICE_SAMPLE_TYPE).where(SERVICE_SAMPLE_TYPE.SAMPLE_TYPE_ID.eq(sampleTypeId).and(
            SERVICE_SAMPLE_TYPE.SERVICE_ID.eq(serviceId)))).map { it.into(ServiceSampleType::class.java) }
    }
}