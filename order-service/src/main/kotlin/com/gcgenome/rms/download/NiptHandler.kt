...
class NiptHandler(private val pdfDrawer: PdfDrawer) {
    fun niptDownload(userId: String, sampleExtension: List<SampleExtension>, patient : Patient): Mono<ByteArray> {
        ...
        pdfDrawer.addTextToPdf(contentStream, patient.name, 195f, 723f, fontGC120, 9f)
        pdfDrawer.addTextToPdf(contentStream, patient.serial, 500f, 723f, fontGC120, 9f)
        val birth = patient.birthDay.toString() + " / " + patient.birthMonth.toString() + " / " +  patient.birthYear.toString()
        pdfDrawer.addTextToPdf(contentStream, birth, 195f, 706.5f, fontGC120, 9f)

        sampleExtension.forEach {
            if (it.extensionId.equals("TA0009"))
                addCheckMarkByEthnicity(contentStream, it.value, 689f, fontGC120, 9f)
        }
